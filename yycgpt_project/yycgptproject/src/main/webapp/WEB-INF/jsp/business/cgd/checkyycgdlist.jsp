...
<script type="text/javascript">
	function yycgdchecksubmit() {
		_confirm('您确认提交审核结果吗?', null, function() {
			var indexs = [];//记录的序号
			...
			if (rows.length > 0) {
				...
				jquerySubByFId('yycgdqueryForm', yycgdchecksubmit_callback,
						null);
			} else {
	...
	var columns = [ [
			{
				...
				formatter : function(value, row, index) {
					return '<input type="hidden" name="yycgdCustoms['+index+'].id" value="'+value+'" /><input type="hidden" name="yycgdCustoms['+index+'].bm" value="'+row.bm+'" />';
				}
			},{
				field : 'mc',
				title : '采购单名称',
				...
				formatter : function(value, row, index) {
					var string = '<select name="yycgdCustoms['+index+'].zt">'
							...
							+ '<option value="4">审核不通过</option>' + '</select>';
					return string
				}
			},{
				field : 'shyj',
				...
				formatter : function(value, row, index) {
					...
					return '<input type="text" name="yycgdCustoms['+index+'].shyj" value="'+val+'"/>';
				}
			},{
				field : 'cjtime',
				title : '建单时间',
...
<BODY>
	<form id="yycgdqueryForm" name="yycgdqueryForm" method="post"
		action="${baseurl}cgd/checkyycgdsubmit.action">
