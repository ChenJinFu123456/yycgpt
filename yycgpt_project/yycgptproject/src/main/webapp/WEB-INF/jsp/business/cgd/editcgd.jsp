...
<script type="text/javascript">
	...
	function yycgdsave() {
		...
		$("#yycgdsaveForm")
				.attr("action", "${baseurl}cgd/editcgdsubmit.action");
	...
	function yycgdsubmit() {
		_confirm('提交后采购单将不允许修改，是否继续?', null, function() {
			$("#yycgdsaveForm").attr("action",
					"${baseurl}cgd/submityycgd.action");
	...
	$(function() {
		$('#yycgdmxlist').datagrid({
			...
			onLoadSuccess : function() {
				$('#yycgdmxlist').datagrid('clearSelections');
			}
		});
	});
...
<BODY>
	...
	<form id="yycgdsaveForm" name="yycgdsaveForm"
		action="${baseurl}cgd/yycgdeditsubmit.action" method="post">
						...
						<TABLE class="toptable grid" border=1 cellSpacing=1 cellPadding=4
							...
							<TBODY>
								...
								<TR>
									...
									<TD class=category width="35%"><fmt:formatDate
											value="${yycgd.cjtime}" pattern="yyyy-MM-dd" /></TD>
									<TD height=30 width="15%" align=right>提交时间：</TD>
									<TD class=category width="35%"><fmt:formatDate
											value="${yycgd.tjtime}" pattern="yyyy-MM-dd" /></TD>

								</TR>
								<TR>
									<TD height=30 width="15%" align=right>联系人：</TD>
									<TD class=category width="35%"><input type="text"
										name="yycgdCustom.lxr" id="yycgd.lxr" value="${yycgd.lxr}"
										style="width: 260px" /></TD>
									<TD height=30 width="15%" align=right>联系电话：</TD>
									<TD class=category width="35%"><input type="text"
										name="yycgdCustom.lxdh" id="yycgd.lxdh" value="${yycgd.lxdh}"
										style="width: 260px" /></TD>
								</TR>
								<TR>
									<TD height=30 width="15%" align=right>采购单状态：</TD>
									<TD class=category width="35%">${yycgd.yycgdztmc}</TD>
									<TD height=30 width="15%" align=right>备注：</TD>
									<TD colspan=3><textarea rows="2" cols="30"
											name="yycgdCustom.bz">${yycgd.bz}</textarea></TD>
								</TR>

								<TR>
									<TD height=30 width="15%" align=right>审核时间：</TD>
									<TD class=category width="35%"><fmt:formatDate
											value="${yycgd.shtime}" pattern="yyyy-MM-dd" /></TD>
									<TD height=30 width="15%" align=right>审核意见：</TD>
									<TD class=category width="35%">${yycgd.shyj}</TD>
								</TR>

								<tr>
									<td colspan=4 align=center class=category><a href="#"
										onclick="yycgdsave()" class="easyui-linkbutton"
										iconCls='icon-save'>保存</a> <!-- 采购单状态为1：未提交、或、4：审核不通过显示提交 -->
										<c:if test="${yycgd.zt =='1' or yycgd.zt=='4'}">
											<a href="#" onclick="yycgdsubmit()" class="easyui-linkbutton"
